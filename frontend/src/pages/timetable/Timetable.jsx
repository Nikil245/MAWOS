...
function ClassCard({ title, entry, empty }) {
  return <section className="card border-l-4 border-primary p-5"><h2 className="text-sm font-semibold text-muted">{title}</h2>{entry ? <><p className="mt-2 text-xl font-bold">{entry.subject_name}</p><p>{entry.subject_code} · {entry.room}</p><p className="text-sm">{entry.faculty} · {entry.section}</p><p className="mt-2 font-semibold">{entry.date} · {entry.start_time}–{entry.end_time}</p></> : <p className="mt-2">{empty}</p>}</section>;
}
...
export function PersonalTimetable({ role, onRefreshScheduled }) {
  ...
  return <><PageHeader title="My timetable" eyebrow="Published academic schedule · Asia/Kolkata" /><div className="grid gap-4 md:grid-cols-2"><ClassCard title="Current class" entry={data.current} empty={data.message} /><ClassCard title="Next class" entry={data.next} empty={data.next_message || 'No remaining published classes.'} /></div><section className="card my-4 p-4"><h2 className="font-bold">Today’s classes</h2>{data.today.length ? data.today.map(e => <p className="mt-2" key={e.id}>{e.start_time}–{e.end_time} · {e.subject_name} · {e.room}</p>) : <p className="mt-2 text-muted">{data.message}</p>}</section><h2 className="mb-3 text-lg font-bold">Weekly timetable</h2>{data.published ? <Weekly entries={data.weekly} /> : <p className="card p-4">No published timetable for the current academic term.</p>}{role === 'faculty' && <Availability />}</>;
}
...
function HodWorkspace({ term }) {
  ...
  const action = (name, body, method = 'POST') => act(async () => { setRun(await request(`/hod/timetable/runs/${run.id}/${name}`, { token, method, body })); setRefresh(r => r + 1); }, name === 'publish' ? 'Timetable published.' : 'Draft updated.');
...
export function PrincipalTimetable() {
  ...
  if (loading) return <LoadingSkeleton />;
  if (error) return <ErrorState error={error} />;
  return <><PageHeader title="Timetable coverage" eyebrow="Institution / Read-only publication overview" />{data.length ? <div className="grid gap-4 md:grid-cols-2">{data.map(r => <section className="card p-4" key={`${r.term_id}-${r.department}`}><h2 className="font-bold">{r.department} · {r.term}</h2><p>{r.published_run_id ? `Published version ${r.published_run_id}` : 'No published timetable'}</p>{r.published_metrics && <p>{r.published_metrics.placed} / {r.published_metrics.required} periods covered</p>}<p className="text-sm text-muted">Latest run: {r.latest_status}</p></section>)}</div> : <p className="card p-4">No academic terms configured.</p>}</>;
}
