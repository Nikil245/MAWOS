...
export function NotificationDrawer({ open, onClose, notifications, loading, error, sessionExpired, retry, onMarkRead, onMarkAllRead }) {
  if (!open) return null;
  return <aside className="fixed inset-y-0 right-0 z-40 w-full max-w-sm border-l bg-white p-5 shadow-xl"><div className="flex justify-between"><h2 className="font-bold">Notifications</h2><button onClick={onClose} aria-label="Close notifications"><X /></button></div>{notifications.some(n => !n.read) && <button className="btn-secondary mt-3" onClick={onMarkAllRead}>Mark all as read</button>}{loading ? <div className="mt-5"><LoadingSkeleton rows={4} /></div> : sessionExpired ? <div className="mt-5"><EmptyState title="Session expired" detail="Sign in again to view notifications." /></div> : error ? <div className="mt-5"><ErrorState error={error} retry={retry} /></div> : notifications.length ? <div className="mt-5 space-y-4">{notifications.map((n, i) => <button type="button" onClick={() => onMarkRead?.(n.id)} disabled={n.read} className={`block w-full border-b pb-3 text-left ${n.read ? '' : 'rounded-lg bg-blue-50 px-3 pt-2'}`} key={n.id || i}><div className="flex items-start justify-between gap-3"><p className="text-sm font-semibold">{n.title}</p><span className="shrink-0 text-xs text-muted">{n.read ? 'Read' : 'Unread'}</span></div><p className="mt-1 text-sm text-muted">{n.message}</p><p className="mt-2 text-xs text-muted">{n.source_agent} · {new Date(n.at).toLocaleString()}</p></button>)}</div> : <EmptyState title="No notifications" />}</aside>;
}
export { Bell };
