...
def test_student_notification_read_state_is_scoped_and_persistent(db):
    student = db.query(User).filter_by(role="student", usn="4MT23AI001").one()
    other = db.query(User).filter_by(role="student", usn="4MT23AI002").one()
    mine = Notification(usn=student.usn, title="Mine", message="Read me", source_agent="test")
    other_note = Notification(usn=other.usn, title="Other", message="Private", source_agent="test")
    ...
    try:
        ...
        db.expire_all(); assert db.get(Notification, mine.id).read is True
