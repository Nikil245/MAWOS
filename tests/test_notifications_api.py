...
def test_personal_notifications_are_visible_only_to_the_authenticated_student(db):
    ...
    _notice(db, "Personal for intended", recipient_user_id=intended.id, usn="4MT23AI001")
...
def test_role_and_department_visibility_uses_server_side_user_identity(db):
    ...
    _notice(db, "AIML faculty", recipient_user_id=aiml_faculty.id)
    _notice(db, "CSE faculty", recipient_user_id=cse_faculty.id)
    _notice(db, "All faculty", recipient_user_id=aiml_faculty.id)
    _notice(db, "All faculty", recipient_user_id=cse_faculty.id)
    _notice(db, "Admin only", recipient_user_id=admin.id)
...
def test_notification_get_is_read_only_and_returns_explicit_fields(db):
    ...
    note = _notice(db, "Unread notice", recipient_user_id=user.id, usn="4MT23AI001", read=False)
    ...
    assert {"id", "title", "message", "notification_type", "route", "created_at", "source_agent", "at", "read", "read_at"} <= item.keys()
